...
<body>
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.result === "good") {
                        ...
                        $('#youNickNameComment').text('이미 존재하는 닉네임 입니다.');
                        nickCheck = false;
                    }
                },
                error: function(request, status, error) {
    ...
    function joinChecks() {
        ...
        // 닉네임 유효성 검사
        const getYouNickName = new RegExp(/^[가-힣||0-9]+$/g);
        ...
        // 확인 비밀번호 공백 검사
        if ($('#youPassC').val() === '') {
            $('#youCComment').text("확인 비밀번호를 입력해 주세요");
            return false;
        }

        // 비밀번호가 동일한지 체크
        if ($('#youPass').val() !== $('#youPassC').val()) {
            $('#youCComment').text("비밀번호가 동일하지 않습니다.");
            return false;
        }
